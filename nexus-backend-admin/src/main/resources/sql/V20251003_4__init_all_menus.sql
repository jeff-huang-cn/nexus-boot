...
(`name`, `permission`, `type`, `sort`, `parent_id`, `path`, `icon`, `component`, `component_name`, `status`, `visible`, `keep_alive`, `always_show`, `creator`) 
SELECT '用户管理', 'system:user:query', 2, 1, @system_dir_id, '/system/user', 'user', 'system/user/index', 'SystemUser', 1, 1, 1, 0, 'system'
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `system_menu` WHERE `permission` = 'system:user:query' AND `type` = 2);

-- 获取用户管理菜单ID
SET @user_menu_id = (SELECT id FROM `system_menu` WHERE `permission` = 'system:user:query' AND `type` = 2 LIMIT 1);
...
INSERT INTO `system_menu` 
(`name`, `permission`, `type`, `sort`, `parent_id`, `path`, `icon`, `component`, `component_name`, `status`, `visible`, `keep_alive`, `always_show`, `creator`) 
SELECT '角色管理', 'system:role:query', 2, 2, @system_dir_id, '/system/role', 'team', 'system/role/index', 'SystemRole', 1, 1, 1, 0, 'system'
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `system_menu` WHERE `permission` = 'system:role:query' AND `type` = 2);

-- 获取角色管理菜单ID
SET @role_menu_id = (SELECT id FROM `system_menu` WHERE `permission` = 'system:role:query' AND `type` = 2 LIMIT 1);
...
INSERT INTO `system_menu` 
(`name`, `permission`, `type`, `sort`, `parent_id`, `path`, `icon`, `component`, `component_name`, `status`, `visible`, `keep_alive`, `always_show`, `creator`) 
SELECT '菜单管理', 'system:menu:query', 2, 3, @system_dir_id, '/system/menu', 'menu', 'system/menu/index', 'SystemMenu', 1, 1, 1, 0, 'system'
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `system_menu` WHERE `permission` = 'system:menu:query' AND `type` = 2);

-- 获取菜单管理菜单ID
SET @menu_menu_id = (SELECT id FROM `system_menu` WHERE `permission` = 'system:menu:query' AND `type` = 2 LIMIT 1);
...
INSERT INTO `system_menu` 
(`name`, `permission`, `type`, `sort`, `parent_id`, `path`, `icon`, `component`, `component_name`, `status`, `visible`, `keep_alive`, `always_show`, `creator`) 
SELECT '代码生成', 'codegen:table:query', 2, 1, @dev_dir_id, '/dev/codegen', 'code', 'dev/codegen/index', 'CodegenTableList', 1, 1, 1, 0, 'system'
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `system_menu` WHERE `permission` = 'codegen:table:query' AND `type` = 2);

-- 获取代码生成菜单ID
SET @codegen_menu_id = (SELECT id FROM `system_menu` WHERE `permission` = 'codegen:table:query' AND `type` = 2 LIMIT 1);
