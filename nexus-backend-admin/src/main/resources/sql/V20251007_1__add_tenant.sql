...
CREATE TABLE IF NOT EXISTS `system_tenant` (
    ...
    `code` VARCHAR(50) NULL DEFAULT NULL COMMENT '租户编码（用于识别租户）',
    `datasource_id` BIGINT NULL DEFAULT NULL COMMENT '数据源ID（关联datasource_config.id）',
    `menu_ids` JSON NULL COMMENT '分配的菜单ID集合，格式：[1, 2, 100, 101, 102]',
    `max_users` INT NOT NULL DEFAULT 10 COMMENT '用户数量（可创建的用户数量）',
    `used_users` INT NOT NULL DEFAULT 0 COMMENT '已使用的用户数量',
